...
const empty: Form = { title: '', description: '', lyrics: '', voice_parts: '', pdf_url: '', video_url: '', audio_url: '' };
...
export default function CompositionsAdminPage() {
  ...
  async function saveAdd() {
    setSaving(true);
    await fetch('/api/admin/compositions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(toBody(form)) });
  ...
  async function saveEdit() {
    if (!editItem) return;
    setSaving(true);
    await fetch(`/api/admin/compositions/${editItem.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(toBody(form)) });
  ...
  async function confirmDelete() {
    ...
    setSaving(true);
    await fetch(`/api/admin/compositions/${deleteItem.id}`, { method: 'DELETE' });
    ...
    <Box sx={{ minHeight: '100vh', backgroundColor: '#F8FAFC' }}>
      <Box sx={{ background: 'linear-gradient(135deg, #1d6db3 0%, #2a7bc4 100%)', py: { xs: 5, md: 7 }, px: 3 }}>
        <Container maxWidth="lg" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
          <Typography variant="h3" component="h1" sx={{ color: 'white', fontWeight: 700 }}>Compositions</Typography>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
        {loading ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {items.length === 0 && (
              <Typography color="text.secondary" sx={{ textAlign: 'center', py: 6 }}>No compositions yet.</Typography>
            )}
          </Box>
        )}
      </Container>
