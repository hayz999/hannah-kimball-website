...
const empty: Form = { title: '', description: '', start_date: '', end_date: '', location: '', event_details_url: '' };
...
export default function EventsAdminPage() {
  ...
  async function saveAdd() {
    setSaving(true);
    await fetch('/api/admin/events', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(toBody(form)) });
  ...
  async function saveEdit() {
    if (!editItem) return;
    setSaving(true);
    await fetch(`/api/admin/events/${editItem.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(toBody(form)) });
  ...
  async function confirmDelete() {
    ...
    setSaving(true);
    await fetch(`/api/admin/events/${deleteItem.id}`, { method: 'DELETE' });
  ...
  const formFields = (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
        <TextField label="Start Date *" type="date" value={form.start_date} onChange={set('start_date')} fullWidth size="small" slotProps={{ inputLabel: { shrink: true } }} />
        <TextField label="End Date" type="date" value={form.end_date} onChange={set('end_date')} fullWidth size="small" slotProps={{ inputLabel: { shrink: true } }} />
      </Box>
      <TextField label="Location" value={form.location} onChange={set('location')} fullWidth size="small" />
      <TextField label="Event Details URL" value={form.event_details_url} onChange={set('event_details_url')} fullWidth size="small" />
    </Box>
  );
    ...
    <Box className="section-pattern" sx={{ minHeight: '100vh' }}>
      <Box sx={{ background: 'linear-gradient(135deg, #1d6db3 0%, #2a7bc4 100%)', py: { xs: 5, md: 7 }, px: 3 }}>
        <Container maxWidth="lg" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
          <Typography variant="h3" component="h1" sx={{ color: 'white', fontWeight: 700 }}>Events</Typography>
            ...
            {items.map(item => (
              <Card key={item.id} elevation={1}>
                <CardContent sx={{ display: 'flex', gap: 2, alignItems: 'center', p: { xs: 2, sm: 2.5 } }}>
                  <EventIcon sx={{ color: 'primary.light', fontSize: 28, flexShrink: 0 }} />
                  <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                    <Typography variant="h6" sx={{ fontWeight: 700, color: 'primary.dark', fontSize: { xs: '1rem', sm: '1.15rem' } }}>
                      {item.title}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      {item.start_date}{item.end_date !== item.start_date ? ` – ${item.end_date}` : ''}{item.location ? ` · ${item.location}` : ''}
                    </Typography>
                  </Box>
      ...
      <Dialog open={adding} onClose={() => setAdding(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          ...
          <Button variant="contained" onClick={saveAdd} disabled={saving || !form.title || !form.start_date}>
            {saving ? 'Saving…' : 'Add'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={!!editItem} onClose={() => setEditItem(null)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          ...
          <Button variant="contained" onClick={saveEdit} disabled={saving || !form.title || !form.start_date}>
