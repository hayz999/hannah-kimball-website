...
export default function ArrangementsAdminPage() {
  ...
  async function saveAdd() {
    setSaving(true);
    await fetch("/api/admin/arrangements", {
      method: "POST",
  ...
  async function saveEdit() {
    if (!editItem) return;
    setSaving(true);
    await fetch(`/api/admin/arrangements/${editItem.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(toBody(form)),
    });
  ...
  async function confirmDelete() {
    ...
    setSaving(true);
    await fetch(`/api/admin/arrangements/${deleteItem.id}`, {
      method: "DELETE",
    });
    setSaving(false);
    setDeleteItem(null);
  ...
  const formFields = (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
      ...
      <TextField
        ...
        value={form.original_composer}
        onChange={set("original_composer")}
        fullWidth
        size="small"
      />
      <TextField
